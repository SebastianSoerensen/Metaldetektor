...
\section*{Signal forstærker}
\begin{circuitikz}[american]
	\draw (0,-2) node[ground]{} to [short,-*]++(0,0) to [vsourcesin,l_=$2\text{kHz}$]++(0,2)
	to [C=$1\mu\text{F}$]++(2,0) to [short,-*]++(0,0) coordinate(N1);
	\draw (N1) to [R=$100\text{k}\Omega$]++(0,-2) to [short,-*]++(0,0);
	\draw (N1) to [R=$100\text{k}\Omega$]++(0,2) to [short,-*]++(0,2) coordinate(N2) to [vsource,l=$5\text{V}$]++(0,2) --++(-4,0) --++(0,-8) --++(6.8,0); % Linje til ground her
	\draw (6,3.5) node[op amp,yscale=-1] (opamp){};
	\draw (opamp.+) to (N2);
	...
	\draw (Vo) --++(1,0) to [open=$V_{out}$] ++(0,0.5);
\end{circuitikz}
