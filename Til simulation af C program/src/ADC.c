...
ISR(ADC_vect)
{
    ...
    // Use Python-generated test vector from FLASH
    sample = pgm_read_word(&test_samples[adc_cur_idx]); // signed sample
    s = (uint16_t)(sample + 512);  // convert to 0-1023 for buffer compatibility
#else
    // Real ADC reading
    s = ADC;
    ...
    // Accumulate into DFT (Goertzel bin at 2 kHz)
    DFT_accum(sample, i);

    // Store raw sample (for debug/plotting if needed)
    adc_buf[buf][i] = s;
